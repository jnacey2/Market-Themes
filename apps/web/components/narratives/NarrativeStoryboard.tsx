...
import type { NarrativeLifecycleState, NarrativeTrendSummary } from "@market-themes/db";
import {
  hasThinEvidence,
  LIFECYCLE_LABELS,
  LifecycleBadge,
...
export function NarrativeStoryboard({ narrative }: { narrative: NarrativeTrendSummary }) {
  ...
  return (
    <div className="shell wide-shell">
      <nav className="context-nav" aria-label="Narrative views">
        ...
        <Link href={narrativeDataPath(narrative.slug)}>Open data view</Link>
        <HowToReadLink />
      </nav>
      ...
      <section className="hero">
              ...
              : hasSignal
                ...
                : !classificationComplete
                  ...
                  : narrative.lifecycleState === "fading"
                    ? `Approved evidence has dropped to zero from a ${narrative.peakDensity.toFixed(1)} peak${narrative.peakDate ? ` on ${narrative.peakDate} (UTC)` : ""}.`
        ...
        <div className="panel">
          ...
          <div className="metric-row">
            <Metric
              label={
                <MetricTerm term="zScore">
                  {narrative.lowHistory ? "Z-score (provisional)" : "Z-score"}
                </MetricTerm>
              }
              value={classificationComplete ? narrative.zScore.toFixed(1) : "—"}
            />
            <Metric
              label={<MetricTerm term="attentionZScore" short />}
              value={classificationComplete ? narrative.attentionZScore.toFixed(1) : "—"}
            />
            <Metric
              label={<MetricTerm term="percentile">Percentile</MetricTerm>}
              value={classificationComplete ? String(narrative.percentileRank) : "—"}
            />
            <Metric
              label={<MetricTerm term="uniqueStories" />}
              value={String(narrative.storyBreadth)}
            />
            <Metric
              label={<MetricTerm term="peak">% of 90d peak</MetricTerm>}
              value={
                !classificationComplete
                  ? "—"
                  : narrative.density <= 0
                    ? "none this week"
                    : `${Math.round(narrative.percentOfPeak)}%`
              }
            />
          </div>
        </div>
